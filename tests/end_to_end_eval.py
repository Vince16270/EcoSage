...
PIVOT_CSV    = os.path.join(OUTPUT_DIR, "pivoted_results.csv")
...
def compute_bleu(ref: str, hyp: str) -> float:
    ...
    return sentence_bleu(r, h, smoothing_function=smoothie)
...
def compute_rouge1_f1(ref: str, hyp: str) -> float:
    ...
    for w in r:
        rc[w] = rc.get(w, 0) + 1
    overlap = sum(1 for w in set(h) if w in rc)
    p = overlap / len(h)
    rcall = overlap / len(r)
    return 2 * p * rcall / (p + rcall) if (p + rcall) else 0.0
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                ...
                print(f"  -> Warning: API call failed (attempt {attempt+1}/{retries}): {e!r}. "
...
def main():
    parser = argparse.ArgumentParser(
        description="Eval: sample N questions, call chat API, pivot on refs, export CSV’s."
    )
    ...
    ref_cols  = sorted([c for c in pivot.columns if c.startswith("reference_")],
                       key=lambda x: int(x.split("_")[1]))
    bleu_cols = sorted([c for c in pivot.columns if c.startswith("bleu_")],
                       key=lambda x: int(x.split("_")[1]))
    wo_cols   = sorted([c for c in pivot.columns if c.startswith("word_overlap_")],
                       key=lambda x: int(x.split("_")[1]))
    em_cols   = sorted([c for c in pivot.columns if c.startswith("exact_match_")],
                       key=lambda x: int(x.split("_")[2]))
    rf_cols   = sorted([c for c in pivot.columns if c.startswith("rouge1_f1_")],
                       key=lambda x: int(x.split("_")[2]))
    ...
    print(f"- Evaluation CSV:   {PIVOT_CSV}")
